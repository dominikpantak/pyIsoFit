...
def dsl_funct(df_lst, keyPressures, keyUptakes, temps=None, compnames=None, henry_constants=None, logplot=False, meth='leastsq', guess=None):
    ...
    def step1(x, y, meth, guess):

        gmod = Model(dsl)
        pars1 = Parameters()

        pars1.add('q1', value=guess['q1'], min=0)
        pars1.add('q2', value=guess['q2'], min=0)
        pars1.add('b2', value=guess['b2'])
    ...
    for i in range(len(compnames)):
        if guess == None:
            guess = get_guess_params("DSL", df_lst[i], keyUptakes, keyPressures)
            print(guess['q1'])
        qtest = step1(*df_dict[compnames[i]], meth, guess)
...
df1 = pd.read_csv('pyIsofit (new Package)\Computational Data (EPFL) CO2.csv')
df2 = pd.read_csv('pyIsofit (new Package)\Computational Data (EPFL) N2.csv')
